	...
	public static void interactEvent(EntityInteract event) {
		if (event.getTarget() instanceof AgeableMob ageableMob && !ageableMob.level().isClientSide()) {
			...
			if (ageableMob.isBaby()) {
				...
				if (stack.is(CommonClass.AGE_LOCKING_TAG) && setLocked(ageableMob, playerIn, stack, true)) {
					event.setCancellationResult(InteractionResult.SUCCESS);
					event.setCanceled(true);
				} else if (stack.is(CommonClass.AGE_UNLOCKING_TAG) && setLocked(ageableMob, playerIn, stack, false)) {
					event.setCancellationResult(InteractionResult.SUCCESS);
					event.setCanceled(true);
				}
			}
